...
pdijkstra ExtractMin(pdijkstra head);
...
pdijkstra ExtractMin(pdijkstra head) {
    pdijkstra ver = NULL;
    while (head != NULL) {
        if (!head->visited && head->distance < INFINITY && (ver == NULL || ver->distance < head->distance)) {
            ver = head;
        }
        head = head->next;
    }
...
int shortsPath_cmd(pnode head, int src, int dest) {
    ...
    pdijkstra u = ExtractMin(dijkstraHead);
    while (u != NULL) {
        pedge edgeIndex = u->node->edges;
        while (edgeIndex != NULL) {
            // relax
            pdijkstra v = getPdijkstra(dijkstraHead, edgeIndex->endpoint->node_num);
            int newDist = u->distance + edgeIndex->weight;
            if (v->distance > newDist) {
                v->distance = newDist;
            }
            edgeIndex = edgeIndex->next;
        }
        u = ExtractMin(dijkstraHead);
    }
    int shortest_way = getPdijkstra(dijkstraHead, dest)->distance;
    if (shortest_way == INFINITY) {
        shortest_way = -1;
    }
